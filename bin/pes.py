...
class PES(object):
	...
	def __init__(self, updateDb=True, window=False):
		self.__temp = 0
		...
		try:
			self.__skipIntro = (configParser.get('pes', 'skipIntro') == '1')
			self.__fontFile = os.path.abspath(self.__baseDir + os.sep + configParser.get('pes', 'fontFile'))
			self.__psxFontFile = os.path.abspath(self.__baseDir + os.sep + configParser.get('pes', 'psxFontFile'))
		...
		self.__checkFile(self.__psxFontFile)
		...
		for c in self.__supportedConsoles:
			...
			try:
				...
				console = Console(c, consoleId, extensions, self.__romsDir + os.sep + c, command.replace('%%BASE%%', self.__baseDir), self.__userDb, self.__imgCacheDir)
                                if defaultInterface and updateDb:
                                        console.updateDatabase()
                                else:
		...
		#menuItems = [MenuItem("Update Database", self.__updateDb), MenuItem("Configure Joystick", self.__loadJoyStickConfiguration)]
		menuItems = [MenuItem("Configure Joystick", self.__loadJoyStickConfiguration)]
		...
		for c in self.__consoles:
			if c.getGameTotal() > 0:
				menuItems.append(MenuItem(c.getName(), self.__loadGamesMenu, c))
	...
	def __poweroff(self):
        ...
        def __updateDb(self):
                if self.__updateDbActive:
                        printMsg("Unable to update PES database: update already in progress!")
                else:
                        self.__updateDbActive = True
                        printMsg("Updating PES database...")
                        for c in self.__consoles:
                                c.updateDatabase()
                        self.__updateDbActive = False
	...
	def run(self, showIntro):
		# start status thread
		#self.__statusThread = StatusThread(self.__tempCommand)
		#self.__statusThread.start()
		...
		if showIntro:
			label = pygame.font.Font(self.__psxFontFile, 30).render("Pi   Entertainment   System", True, (255, 255, 0))
			text_rect = label.get_rect(center=(self.__screenWidth / 2, self.__screenHeight / 2))
			FADE_IN_TIME = 5
			FADE_IN_EASING = lambda x: x
			last_state_change = time.time()
			state_time = 0
			while state_time < FADE_IN_TIME:
				...
				surf2.blit(label, (0, 0))
				self.__screen.blit(surf2, text_rect)
				pygame.display.flip()
		...
		while ok:
			...
			#self.__footer.setField("Temp", self.__statusThread.getTemp())
										...
										for i in range(0, self.__joystickTotal):
											...
											for j in self.__joysticks:
												if j.isMatch(js.get_name()):
													...
													#break
										f.write("input_exit_emulator_btn = \"%s\"\n" % self.__joysticks[firstJs].getButtonValue(JoyStick.BTN_EXIT))
...
class StatusThread(threading.Thread):
	...
	def __init__(self, tempCommand):
		threading.Thread.__init__(self)
		self.__temp = 0
		self.__sleep = 10
		self.__run = True
		self.__tempCommand = tempCommand
		printMsg("StatusThread.__tempCommand = %s" % tempCommand)
	...
	def run(self):
		while self.__run:
			...
			try:
				temp = float(subprocess.check_output(self.__tempCommand, shell=True)[:-1])
				temp /= 1000
				self.__temp = "%.2fC" % temp
			except subprocess.CalledProcessError, e:
				printMsg("CalledProcessError: %s", e.args)

			#printMsg("Status thread sleeping")
...
class JoyStick(object):
	...
	def getRetroArchConfig(self, index):
		...
		cfg += "%s_b_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_B])
		cfg += "%s_a_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_A])
		cfg += "%s_y_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_Y])
		cfg += "%s_x_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_X])
		cfg += "%s_l_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_SHOULDER_LEFT])
		cfg += "%s_r_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_SHOULDER_RIGHT])
		cfg += "%s_start_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_START])
		cfg += "%s_select_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_SELECT])
		cfg += "%s_up_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_UP])
		cfg += "%s_down_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_DOWN])
		cfg += "%s_left_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_LEFT])
		cfg += "%s_right_btn = \"%s\"\n" % (prefix, self.__btnMap[JoyStick.BTN_RIGHT])
		return cfg
...
class Console(object):
	...
	def updateDatabase(self):
		...
		try:
			...
			con.row_factory = sqlite3.Row
			cur = con.cursor()
			cur.execute('UPDATE `games` SET `exists` = 0 WHERE `console_id` = %d;' % self.__id)
			con.commit()
			self.__games = []
			files = []
			for e in self.__extensions:
				for f in glob.glob('%s%s*%s' % (self.__romDir, os.sep, e)):
					if os.path.isfile(f):
						files.append(f)

			files.sort()
			for f in files:
				name = os.path.split(self.__romDir + os.sep + f)[1]
				for e in self.__extensions:
					name = name.replace(e, '')
				...
				if row == None:
					...
					# now grab thumbnail
					#obj = { 'api_key': apiKey, 'filter': "platforms:%d,name:%s" % (self.__id, name), 'field_list': 'name,id', 'format': 'json' }
					obj = { 'game_name': '%s' % name, 'platform_id': self.__id }
					data = urllib.urlencode(obj)
					#printMsg("loading: " + url + 'games/?' + data)
					#response = urllib2.urlopen(url + 'games/?' + data)
					...
					if gameId != None:
						...
						#obj = { 'api_key': apiKey, 'format': 'json', 'field_list': 'image' }
						obj = { 'game_id': gameId }
						data = urllib.urlencode(obj)
						#response = urllib2.urlopen(url + 'game/' + str(gameId) + '/?' + data)
						response = urllib2.urlopen(url + data)
						results = json.loads(response.read())
						...
						if results['results']['image']['small_url']:
							...
							thumbPath = self.__imgCacheDir + os.sep + str(gameId) + extension
							urllib.urlretrieve(imgUrl, thumbPath)
					else:
						printMsg("Could not find game info for %s " % name)
						gameId = -1
					cur.execute('INSERT INTO `games`(`exists`, `console_id`, `name`, `game_path`, `api_id`, `cover_art`) VALUES (1, %d, "%s", "%s", %d, "%s");' % (self.__id, bestName, f, gameId, thumbPath))
					if thumbPath == 0:
						self.__games.append(Game(bestName, f, self, None))
					else:
						self.__games.append(Game(bestName, f, self, thumbPath))
				else:
					printMsg("no need to download for %s" % name)
					coverArt = None
					if row['cover_art'] != '0':
						coverArt = row['cover_art']
						# need to add check for missing cached image
					self.__games.append(Game(row['name'], row['game_path'], self, coverArt))
...
class GamesMenu(Panel):
	...
	def __init__(self, console, width, height, font, fontSize, colour, bgColour, nocoverImage):
		...
		#self.__thumbWidth = 252
		#self.__thumbHeight = 300
		...
		self.__pageTotal = int(len(self.__entries) / self.__visibleItems)
	...
	def draw(self, x, y):
		...
		if self.isActive() and self.__redraw:
			...
			while i < self.__visibleItems + self.__startIndex and i < len(self.__entries):
			...
			if event.type == KEYDOWN:
				if event.key == K_UP:
					...
					if self.__selected < 0:
						self.__selected = len(self.__entries) - 1
					...
					if self.__selected > self.__startIndex + self.__visibleItems - 1:
						self.__startIndex = self.__getStartIndex(self.__selected)
						if self.__startIndex < len(self.__entries):
							if self.__selected > len(self.__entries) - 1:
								self.__selected = self.__startIndex
						else:
							self.__startIndex = 0
							self.__selected = 0
					elif self.__selected > len(self.__entries) - 1:
						self.__startIndex = 0
						self.__selected = 0
					self.__entries[self.__selected].setSelected(True)
					self.__redraw = True
...
class AboutPanel(Panel):
	...
	def draw(self, x, y):
		...
		if self.isActive() and self.__redraw:
			...
			for l in self.getLabels(['Pi Entertainment System version %s' % VERSION_NUMBER, ' ', 'Author: %s' % VERSION_AUTHOR, ' ', 'Released: %s' % VERSION_DATE, ' ', 'Licensed under version 3 of the GNU Public License (GPL)'], self.__font, self.__colour, self.getBackgroundColour(), self.getWidth() - x, self.getHeight()):
...
class JoyStickConfigurationPanel(Panel):
	...
	def __init__(self, width, height, font, fontSize, colour, bgColour, configFile):
		...
		self.__redraw = True
		self.__lastBtn = None
		self.__prompts = ['Start', 'Select', 'Up', 'Down', 'Left', 'Right', 'A', 'B', 'X', 'Y', 'Shoulder L', 'Shoulder R', 'Exit Game']
		self.__btns = [JoyStick.BTN_START, JoyStick.BTN_SELECT, JoyStick.BTN_UP, JoyStick.BTN_DOWN, JoyStick.BTN_LEFT, JoyStick.BTN_RIGHT, JoyStick.BTN_A, JoyStick.BTN_B, JoyStick.BTN_X, 
JoyStick.BTN_Y, JoyStick.BTN_SHOULDER_LEFT, JoyStick.BTN_SHOULDER_RIGHT, JoyStick.BTN_EXIT]
		self.__answers = []
		i = 0
...
class Menu(Panel):
	...
	def __init__(self, entries, width, height, font, fontSize, colour, bgColour):
		...
		self.__startIndex = 0
		self.__visibleItems = (self.getHeight() - self.__marginBottom - self.__marginTop) / (self.__fontHeight + self.__menuItemGap)
		if self.__visibleItems > len(self.__entries):
			self.__visibleItems = len(self.__entries) 
			...
			if event.type == KEYDOWN:
				if event.key == K_UP:
					if self.__selected - 1 >= 0:
						...
						self.__entries[self.__selected].setSelected(False)
						self.__selected = len(self.__entries) - 1
						self.__startIndex = len(self.__entries) - self.__visibleItems
						self.__entries[self.__selected].setSelected(True)
				elif event.key == K_DOWN:
					if self.__selected + 1 < len(self.__entries):
						self.__entries[self.__selected].setSelected(False)
						self.__selected += 1
						self.__entries[self.__selected].setSelected(True)
...
class GameMenuItem(MenuItem):
	def __init__(self, game):
		super(GameMenuItem, self).__init__(game.getName())
		self.__game = game
	...
	def getThumbnail(self, width, height):
                if self.__thumbnail == None:
                        if self.__game.getImagePath() != None:
                                self.__thumbnail = scaleImage(pygame.image.load(self.__game.getImagePath()).convert(), (width, height))
...
if __name__ == "__main__":
	...
	parser.add_argument('-u', '--updatedb', help='Update games database', dest='updateDb', action='store_true')
	parser.add_argument('-i', '--hideintro', help='Hide intro', dest='showIntro', action='store_false')
	...
	try:
		...
		printMsg("CEC module not found, disabling CEC functions")
	...
	printMsg("script dir is: %s" % scriptDir)
	printMsg("loading GUI...")
	pes = PES(updateDb, args.window)
	pesActive = True
	command = pes.run(showIntro)
	pesActive = False
	...
	with open(commandFile, 'w') as f:
		if command:
			...
			f.write("%s\n" % command)
			#f.write("echo sleeping\nsleep 3\n")
			f.write("exec %s/pes.sh -i %s\n" % (scriptDir, launchArgs))
		else:
			f.write('exit')
	sys.exit(0)
	...
	#	del pes
	#	if command:
	#		printMsg("running %s" % command)
			#sys.exit(0)
	#		try:
	#			rtn = subprocess.call(command, shell=True)
	#			printMsg("waiting for 2s...")
        #                       time.sleep(2)
        #               except Exception, e:
        #			printMsg(e.args[0])
	#	else:
