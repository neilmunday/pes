...
class PESApp(object):
	...
	def playGame(self, game):
		...
		if emulator == "RetroArch":
			...
			# set audio device for RetroArch
			with open(userRetroArchAudioDeviceConfFile, "w") as f:
				f.write("# THIS FILE IS AUTOMATICALLY GENERATED BY PES!\n")
				if self.config.audioDevice:
					f.write("audio_device = \"%s\"" % self.config.audioDevice)
	...
	def saveSettings(self):
		...
		try:
			...
			if len(audioDevicesToggled) > 0:
				self.app.config.audioDevice = audioDevicesToggled[0]
				self.app.config.set("Audio", "device", audioDevicesToggled[0].getText())
