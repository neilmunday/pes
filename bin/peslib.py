...
class PES(object):
	...
	def __createRetroArchJoystickCfg(self):
		...
		if self.__joystickTotal > 0:
			logging.info("generating RetroArch joystick configuration: %s" % self.__retroarchJoysticksConfigFile)
	...
	def run(self):
		...
		for c in self.__consoles:
			...
			try:
				...
				print e.message
				#logging.error("an error occurred whilst trying to open console url: %s" % e.args)
