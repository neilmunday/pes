...
from pes.retroachievements import RetroAchievement
from pes.ui import *
import pes.event
...
class PESApp(object):
	...
	def playGame(self, game):
		...
		if emulator == "RetroArch":
			...
			if self.retroAchievement == None:
				s += "cheevos_enable = false\n"
			else:
				s += "cheevos_username = %s\n" % self.retroAchievement.getUsername()
	...
	def setRetroAchievements(self, user, password, apiKey):
		...
		self.retroAchievement = RetroAchievement(user, apiKey)
...
class AchievementsScreen(Screen):
	...
	def __init__(self, app, renderer, menuRect, screenRect):
		menu = None
		if app.retroAchievement:
			menu = Menu([MenuItem("Browse"), MenuItem("Update")])
		else:
 ...
 self.app.titleFont, self.app.textColour, fixedWidth=self.wrap))
		...
		if app.retroAchievement:
			...
			self.__descriptionLabel = self.addUiObject(Label(self.renderer, self.screenRect[0] + self.screenMargin, self.__titleLabel.y + self.__titleLabel.height + 30, "To track your achievements in your games, please enter your www.retroachievements.org username, password and API key into \\pes\config\pes\pes.ini or %s and then reload PES." % userPesConfigFile, self.app.bodyFont, self.app.textColour, fixedWidth=self.wrap))
	...
	def __loadAchievements(self, gameDict):
		...
		achievements = self.app.retroAchievement.getGameInfoAndUserProgress(self.app.retroAchievement.getUsername(), int(gameDict["GameID"]))
		menu = Menu([])
	...
	def processEvent(self, event):
		...
		if oldMenuActive and event.type == sdl2.SDL_KEYDOWN and event.key.keysym.sym == sdl2.SDLK_BACKSPACE:
			if selectedText == "Browse" and self.__achievementsList.isVisible():
